...
namespace ImGui {
bool SpacebarToggle(const char* label, bool* v) {
    ImGuiWindow* window = GetCurrentWindow();
    ...
    float height = 23.0f;   // was 65, then 44 - further reduced per compact-panel request
    ...
    float press_anim = ImLerp(
        window->StateStorage.GetFloat(id + 10, 1.0f),
        held ? 0.96f : 1.0f,
        g.IO.DeltaTime * 15.0f
    );

    window->StateStorage.SetFloat(id + 10, press_anim);

    ImDrawList* draw = window->DrawList;

    ImVec2 center = bb.GetCenter();

    auto ScaleVec = [&](ImVec2 p) {
        return center + (p - center) * press_anim;
    };

    float time = (float)g.Time;

    int flicker = (int)(20.0f * sinf(time * 8.0f));

    // =========================================
    // VAPORWAVE HOT PINK & MAGENTA NEON COLORS
    // =========================================
    ImU32 col_fire1 = IM_COL32(
        255,
        50 + flicker / 2,
        150,
        255
    ...
    if (anim > 0.01f) {
        draw->AddRectFilledMultiColor(
            ...
            IM_COL32(23, 5, 15, 255),
            IM_COL32(215, 30, 115, 120),

            IM_COL32(45, 10, 30, 180),
            IM_COL32(255, 50, 150, 120)
        );

        for (int i = 0; i < 3; i++) {
            draw->AddRect(
                ScaleVec(bb.Min - ImVec2(i, i)),
                ScaleVec(bb.Max + ImVec2(i, i)),
    ...
    float pulse = 2.0f + 1.2f * sinf(time * 6.0f);
    ...
    if (anim > 0.01f) {
        ...
        const int ember_count = 30;

        for (int i = 0; i < ember_count; i++) {
            float seed = i * 32.123f;
            ...
            draw->AddLine(
                ...
                (ember_col & 0x00FFFFFF) | 0x20000000,
    ...
    float knob_x = ImLerp(
        bb.Min.x + radius + margin,
        bb.Max.x - radius - margin,
        anim
    );

    ImVec2 knob_pos = ScaleVec(
        ImVec2(
            knob_x,
